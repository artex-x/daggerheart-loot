...
ALL.forEach((x) => {
  ['en', 'ende', 'ru', 'rud'].forEach((k) => {
    ...
    ok(
      !/(Melee|Close|Far|Proficiency|Evasion|Spellcast|advantage|disadvantage)(range|on|of|to|with)\b/.test(
        v
      ),
...
Object.keys(DATA).forEach((table) => {
  ...
  if (table === 'community') {
    ...
    DATA[table].forEach((x) => {
      (byC[x.community] = byC[x.community] || []).push(x);
    });
    ok(Object.keys(byC).length === 9, 'communities are not 9');
    Object.entries(byC).forEach(([community, rows]) =>
      ROLL_POOLS.push([table + '/' + community, rows])
    );
  } else if (table === 'voa') {
    ...
    const byT = {};
    DATA[table].forEach((x) => {
      (byT[x.tier] = byT[x.tier] || []).push(x);
    });
    ok(
      Object.keys(byT).length === 6,
      'Vault of Ages sections are not 6, but ' + Object.keys(byT).length
    );
...
ALL.forEach((x) =>
  ok(ROLLED.has(x) === (x.roll != null), x.id + ': roll number does not match pool membership')
);
ROLL_POOLS.forEach(([name, rows]) => {
  const rolls = rows.map((x) => x.roll);
  ok(
    rolls.every((n) => Number.isInteger(n) && n > 0),
    name + ': roll number is not a positive integer'
  );
  ok(new Set(rolls).size === rolls.length, name + ': duplicate roll number');
  ok(
    [...rolls].sort((a, b) => a - b).join() === rows.map((_, i) => i + 1).join(),
...
Object.keys(REFS).forEach((k) => {
  ok(
    ALL.some((x) => (x.refs || []).indexOf(k) >= 0),
    'card ' + k + ' is not mentioned by any record'
  );
  ['en', 'ende', 'ru', 'rud', 'url'].forEach((f) =>
    ok(!!REFS[k][f], 'card ' + k + ' is missing field ' + f)
  );
});
...
EQ.forEach((x) => {
  ...
  if (e.t === 'armor') {
    ok(
      ...
      x.id + ': armour is missing thresholds or a score'
    );
    ok(e.th[0] < e.th[1], x.id + ': thresholds are not ascending');
    ok(
      e.tr === null && e.rg === null && e.bu === null,
      x.id + ': armour has weapon fields filled in'
    );
  } else {
  ...
  if (e.line) {
    ...
    ok(
      byId[e.line] && byId[e.line].eq.t === e.t,
      x.id + ': upgrade line is a different equipment type'
    );
  }
});
...
ALL.forEach((x) => {
  ...
  ok(
    fs.existsSync(path.join(ROOT, 'og', x.img.replace(/\.webp$/, '.jpg'))),
...
ALL.filter((x) => x.img).forEach((x) => {
  (shared[x.img] = shared[x.img] || []).push(x);
});
Object.keys(shared).forEach((img) => {
  ...
  ok(
    lines.size === 1,
    'picture ' + img + ' is shared by unrelated records: ' + rows.map((r) => r.en).join(', ')
  );
});
...
ALL.forEach((x) => {
  [
    [
      'ende',
      x.ende,
      /(?:^|[.\s])((?:[A-ZÄÖÜÉ][A-ZÄÖÜÉ’'-]{2,}\s+){0,4}[A-ZÄÖÜÉ][A-ZÄÖÜÉ’'-]{2,})\s*$/
    ],
  ...
  if (e && r)
    ok(
      ENDS.test(e) === ENDS.test(r),
      x.id +
        ': sentence ends differently — EN «…' +
        e.slice(-30) +
        '» / RU «…' +
        r.slice(-30) +
        '»'
...
ALL.forEach((x) => {
  ...
  ok(
    !(HEADWORDS.indexOf(first) >= 0 && x.ru.split(' ').length > 1),
    x.id + ': name starts with a header word — ' + x.ru
  );
  ok(
    !/^(Name|Trait|Range|Damage|Burden|Feature|Base)\b/.test(x.en),
...
fs.readdirSync(path.join(ROOT, 'img')).forEach((f) => {
  ...
  const sum = crypto
    .createHash('md5')
    .update(fs.readFileSync(path.join(ROOT, 'img', f)))
    .digest('hex');
