...
const MSG = {
  ...
  cloudPush:
    'Blocked: a cloud session pushes only its own task branch, never `main`; the owner squash-merges it onto `main` (`CLAUDE.md`). Push the current branch by name, for example `git push -u origin <current branch>`, without --all, --mirror, --tags or --delete.',
...
// pushes its task branch after every green commit, and the owner
