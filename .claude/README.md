...
| `PreToolUse` | `Edit\|MultiEdit\|Write\|NotebookEdit` | `edit-guard.mjs` | Blocks writes to `data.json`, `catalog.csv`, `i/*.html`, `pages/*.html`, `dist/`, `package-lock.json`, `tests/app/snapshots/**`. | **block** |
...
content, so a `npm run build` run before the check no longer satisfies the
stale-`dist/` guard - rebuild after the check, before any `dist/`-driven
suite or golden probe.
...
`run.mjs` (which imports `sharp` lazily) and `icons.mjs` (the app icons)
are not - their honest proof is the filesystem and the encoder, not a unit
test. No skill was
