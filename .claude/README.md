...
working tree" (`CLAUDE.md`). Handed to Phase 8 as an open question
(`issues/47/handoff.md`, "Phase 8 opening inputs"): whether the
still-surviving heavy runs (`run-all.js`'s pool, the four `golden.js`
shards) need a lock of their own, now that the class of collision the old
one caught can recur.
...
`run.mjs` (the only file that imports `sharp`, lazily) is not - its honest
proof is the filesystem and the encoder, not a unit test. No skill was
