...
const T = {
  ru: {
    ...
    pages: {
      ...
      search: ['Поиск', 'Поиск по всем 1091 позиции сразу — добыча, расходники и снаряжение, на русском и на английском.']
    },
  },
  en: {
    ...
    pages: {
      ...
      search: ['Search', 'Search all 1091 entries at once — loot, consumables and equipment, in Russian and English.']
    },
  }
};
const t = () => T[S.lang];
...
/* Кость на кнопке рисуется одними линиями - тело и грани, без заливки: кнопка
   просвечивает сквозь неё, как сквозь остальные значки в ряду. Пути и рамка
   взяты из `card/die-dN-bw.svg` слово в слово, а `tests/derived.js` сверяет их
   с файлами: прочитать файл на лету нельзя, `fetch` из `file://` запрещён. */
const DIE_PATH = {
  d4: ['0 0 21.5 23.5',
    'M0.75 22.75V0.75L20.75 11.75L0.75 22.75Z',
    'M20.75 11.75H7.70652M0.75 22.75V0.75L20.75 11.75L0.75 22.75ZM7.70652 11.75L0.75 0.75M7.70652 11.75L0.75 22.75'],
  d6: ['0 0 30.25 34.75',
    'M15.125 0.875L29.375 9.55921V26.0592L15.125 33.875L0.875 26.0592V9.55921L15.125 0.875Z',
  ...
  d8: ['0 0 31.75 34.75',
    'M30.875 26.356L15.875 33.875L0.875 26.356V9.64715L15.875 0.875L30.875 9.64715V26.356Z',
  ...
  d10: ['0 0 36.25 34.75',
    'M18.125 0.875L35.375 14.2534V19.6047L18.125 33.875L0.875 19.6047V14.2534L18.125 0.875Z',
  ...
  d12: ['0 0 23.25 24.25',
    'M18.3472 21.2039L22.625 15.7566V8.49342L18.3472 3.04605L11.625 0.625L4.90278 3.04605L0.625 8.49342V15.7566L4.90278 21.2039L11.625 23.625L18.3472 21.2039Z',
  ...
  d20: ['0 0 23.15 25.15',
    'M22.575 18.8908V6.25921L11.575 0.575L0.575 6.25921V18.8908L11.575 24.575L22.575 18.8908Z',
...
/* Форма у каждой кости своя, и узнать по ней, что бросают, проще, чем прочитать
   «d12». Сотка катается парой десятков, так что и знак у неё десятки; где кости
   не бывает - колесо. Цвет берётся у кнопки: знак стоит и на золотом, и на
   тёмном, и своя заливка пропала бы на одном из двух. */
function dieIcon(n){
  const p = DIE_PATH['d' + (n === 100 ? 10 : n)];
  if (!p) return ICON_WHEEL;
  return '<svg viewBox="' + p[0] + '" class="dieicon" fill="none" stroke="currentColor"' +
    ' stroke-linejoin="round"><path d="' + p[1] + '"/><path d="' + p[2] + '"/></svg>';
}
...
function cardArt(name){
  /* В чёрно-белом у костей нет деления на физический и магический: цвет и был
     единственным, чем они отличались. Правило именно про кости: рамка
     магического оружия отличается ещё и рисунком, и общее «убрать -mag»
     подменяло её рамкой физического. */
  if (S.printBW) name = name.replace(/^(die-d\d+)-(phy|mag)$/, '$1');
