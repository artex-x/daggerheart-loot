...
/* Кость на кнопке - тот же вектор, что печатается на карточке: светлое тело с
   гранями. Он один читается и на золотой кнопке, и на тёмной панели, а золотая
   кость на золотом пропадала бы. Сотка катается парой десятков, так что и знак
   у неё десятки; где кости не бывает - колесо. */
function dieIcon(n){
  const d = n === 100 ? 10 : n;
  return DIE_ART['d' + d]
    ? '<img class="dieicon" src="' + CARD_ART + 'die-d' + d + '-bw.svg" alt="">'
    : ICON_WHEEL;
}
