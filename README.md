...
Заметки **входят в ссылку** и восстанавливаются из неё вместе со списком — то есть их прочтёт любой, кому вы её отправите. Чтобы отправить игрокам список без них, есть кнопка **Скопировать текст**: в неё заметки не попадают, как и в отправку отдельной карточки.

В коде заметки живут рядом с количеством и ценой: `l.note` для списка и `l.meta[id].note` для позиции. В ссылке они лежат после строки с позициями и отделены управляющими символами `\x1e` и `\x1f`, которых нет в наборе с клавиатуры, — поэтому перевод строки внутри заметки ничего не ломает.
