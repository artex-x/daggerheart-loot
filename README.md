...
Двуязычный (RU/EN) генератор добычи для настольной ролевой игры Daggerheart. Статический сайт без сборки: 449 предметов и расходников плюс 379 единиц снаряжения, у каждого — название, описание и картинка.
...
- **Поиск** — по всем 828 записям сразу, по названиям, описаниям и характеристикам, на обоих языках
...
img/*.webp                  531 картинка, 640×640, ~20 МБ
og/*.jpg                    те же картинки в JPEG для превью ссылок, ~20 МБ
i/*.html                    828 страниц-заглушек с Open Graph
...
```js
{
  id:'q17', src:'core', kind:'equip',
  en:'Katana',  ende:'Quick: When you make an attack…',
  ru:'Катана',  rud:'Быстрое: Когда вы совершаете атаку…',
  img:'',
  eq:{ t:'weapon', tier:1, tr:'agility', rg:'melee',
       dmg:'d10+3', dt:'phy', bu:2, as:null, th:null, line:'katana' }
}
```
...
| `tier` | ранг 1–4; `0` — снаряжение из Wondrous Loot, у которого ранга нет |
...
| `line` | ключ линейки улучшений; пусто у уникальных вещей |
...
`line` связывает Катану, Улучшенную, Продвинутую и Легендарную Катану в одну линейку: у всех четырёх `line:'katana'`. У вещей, которые существуют в единственном виде, поле пустое — на этом и построен фильтр «Улучшаемые / Уникальные».

Одиннадцать записей из Wondrous Loot — тоже оружие. Они остались в `items.wondrous` со своим номером броска и просто получили такой же блок `eq`, поэтому показываются и в таблице Wondrous, и в таблицах снаряжения, оставаясь одной записью.
