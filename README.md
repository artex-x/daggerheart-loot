...
- **Фильтры «Источник» и «Показать»** — переключатели с множественным выбором: **Core / Hope & Fear** и **Предметы / Расходники**, по умолчанию отмечено всё. Общие для всех режимов, где может выпасть и то и другое: выбор запоминается при переходе между разделами. Снять последний оставшийся переключатель нельзя
