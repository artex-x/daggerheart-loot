...
| `dhloot.prefs.v1` | вид таблиц (списком/сеткой), фильтр типов, источники броска, редкость, сообщество |
| `dhloot.warn.v1` | что предупреждение о хранении закрыто |

Граница проходит между «как приложение должно себя вести» и «что я спрашиваю прямо сейчас». Первое запоминается: вид таблицы, какие типы показывать, из каких книг бросать. Второе — нет: число в поле броска, текст в поиске, открытая справка, выделенные позиции. Восстанавливать их через сутки значило бы выдавать шум за память.

Настройки читаются как чужие данные: каждое поле проверяется отдельно, негодное молча заменяется умолчанием, а набор переключателей, где не осталось ни одного включённого, игнорируется целиком — иначе страница навсегда осталась бы пустой.
...
Назначить можно любой из семи разделов и любую таблицу по имени (`#/tables/eq_weapon`). Карточку предмета или список — нельзя: это снимок, который со временем разойдётся с данными, поэтому кнопка там не появляется.
