...
- **Sweet Moss** — применена эррата от 9 сентября 2025 (стр. 133): `1d10 Ран или 1d10 Стресса`. В допечатном PDF корника было `1d4`.
- **Alternate / Rare / Надежда, строка 4** — в исходном PDF там стоит `Glider`, который уже занимает строку 1, а `Portal Seed` не использован нигде. Заменено на `Portal Seed`.
- Расхождения имён между альтернативными таблицами и книгами (`Selt-Tying Rope` → `Self-Tying Rope`, `Nightwalker's Ring` → `Nighthawker's Ring`, `Gaiman's Backpack` → `Gadiman's Backpack`, `Esher's Mirrorball` → `Escher's Mirrorball`, `Formed Serum` → `Formoid Serum`, `Necromancer's Bell` → `Necroprancer's Bell` и другие) сведены к написанию из книг.
- **Wondrous Loot** — 119 записей вместо d100. Кнопка `d100` бросает 1–100, кнопка `d119` — по всей таблице.
