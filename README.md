...
Блок «Списки живут только в этом браузере» стоит в одном и том же месте на обеих страницах — под шапкой, над содержимым — и закрывается крестиком. Отметка о закрытии лежит в `localStorage` под ключом `dhloot.warn.v1`, поэтому больше не появляется. Второй вариант блока — «браузер блокирует локальное хранилище» — крестика не имеет: запомнить закрытие всё равно негде, а именно из-за этого списки и пропадают.
