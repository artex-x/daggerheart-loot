<script lang="ts">
  /* The "data did not load" line - the rewrite's own state, not a live one.
   *
   * `app.js:7` dereferences `window.LOOT.items` and throws, so with `data.js`
...
<p class="miss">{@render children()}</p>
