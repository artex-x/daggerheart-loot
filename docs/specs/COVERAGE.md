...
still run until Phase 7 deletes it. Six more, under `tests/app/` (`app/sweep`,
`app/golden`, `app/contracts`, `app/states`, `app/typo`, `app/hues`), test the
...
real network, or a real clipboard. One more again, `tests/stub.js`, tests
neither app: it asserts the generated `i/*.html` share stubs do not scroll
sideways. Each old-app suite below carries its
