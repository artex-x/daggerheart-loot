...
`app/src/ports/fake-cloud.ts` and hands its `CloudPort` to `browserEnv`. The
...
`dhloot-fake-cloud`, and also when no `dist-test/assets/*.js` does, so a
renamed marker cannot pass silently. The mode is gated on `command ===
...
- `?as=<user>` in the address signs that seed user in, read once at boot;
  ...
  })` sets it. An unknown user throws at boot, so the page never mounts and
  the driver's wait fails loudly - a typo never passes as signed out.
...
| `ports/fake-cloud.test.ts` | `ports/cloud.contract.ts`, the contract every `CloudPort` meets - R1 covers `auth`: signed out at start, a sign-in as the seed's default user notifying once, the seed's identities in order, unlink allowed then refused for the last identity, link, sign-out notifying `null`, a port made as `gm2`, delete account - run over the fake here and over the real adapter in layer 4; plus the fake's own refusals, an unknown `?as=` user throwing, and `installFakeCloud` |
