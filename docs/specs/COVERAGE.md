...
- `tests/db/` - layer 3, the database, run by `npm run check:db`, not by
  ...
  `auth.users` - is seen by the role and rolled back) and pins three
  ...
  migration revokes it). `delete-account.test.mjs` holds `delete_account()`:
  ...
  refused. `reversibility.test.mjs`
...
| `ports/fake-cloud.test.ts` | `ports/cloud.contract.ts`, the contract every `CloudPort` meets - R1 covers `auth`: signed out at start with no redirect result, a sign-in as the seed's default user notifying once, the seed's identities in order, unlink allowed then refused for the last identity, link, sign-out notifying `null`, a port made as `gm2`, delete account - run over the fake here and over the real adapter in layer 4; plus the fake's own refusals, its two test options (`linkError` refuses a link and changes nothing, `returned` is the redirect result as given), an unknown `?as=` user throwing, and `installFakeCloud` |
...
| `ports/supabase.test.ts` | the real adapter over a stub supabase-js client: the PKCE client options, the session and identity mapping, the code exchanged before the session is read, every refusal mapped (`identity_already_exists`, `single_identity_not_deletable`, a throw), the way back saved before the provider call, sign-out scopes, delete then a local sign-out whatever it answers, auth changes forwarded |
| `ports/lazy-cloud.test.ts` | the lazy port: loaded once on first use, every method delegated, a subscription made when the port arrives or cancelled before, and a failed load answering signed out and refusing |
| `components/accountPage.test.ts` | `#/account` over the fake cloud: the chooser and its consent links in both languages, sign-in, the redirect state, only the title while the session is unknown, the four sections as `gm1`, disconnect and its hint, connect and `redirecting`, the already-linked alert from `link()` and from the redirect, sign out and everywhere (`'global'`), delete with the typed word (trimmed, any case), cancel, every refusal's toast, an account with no provider identity or no email, English, and axe in each state |
...
- The provider redirect and the real Supabase client are proven only
  against a stub client (`ports/supabase.test.ts`) until the hosted E2E
  layer runs them, and OAuth itself - the consent screens, a cancelled
  consent, "Sign out everywhere" reaching another device - only by the
  owner's manual check at a release's closeout.
