...
- The fake is in memory, seeded from `app/src/ports/fake-cloud-seed.ts`: two
  ...
  (Google only). The seed grows with each release's port member. It also
  holds cloud lists - `gm1` a full list with both notes, a player and a GM
  share link and prices in coins, an empty list and an old one; `gm2` one
  list - whose times are offsets back from boot, so "edited N ago" reads
  the same on every run; `fake-cloud.test.ts` pins them to the database's
  bounds. The fake's `lists` repository serves them per user, hands out
  `uuid(5000)`, `uuid(5001)`, ... from `newId()` (so a golden's address is
  the same on every run), and refuses the 51st list and the 101st entry of a
  list with the `limit` answer (option `limits` lowers them for a unit test).
  Its failure switch, option `offline` or `setOffline(on)`, makes `list()`
  fail and every list write answer `network`; the switch is not part of
  `CloudPort`.
...
- `window.__dhlootFake` is the port itself, its `setOffline` and its
  `marker`, in the test build only - how a states case reads the session.
  The driver's `fake(path, ...args)` calls the function at a dotted path on
  it in the page and awaits it (`d.fake('setOffline', true)`,
  `d.fake('auth.signIn', 'google')`, `d.fake('lists.list')`). Its `go(hash)`
  sets `location.hash` in the page as a pasted link does, and `shownAgain()`
  dispatches `visibilitychange`, the shown-again signal; each then settles.
- The test build always has the fake, so signed out it draws the sign-in
  prompt where a list would be made; the golden and states cases that make a
  list open `as gm2` (`docs/DECISIONS.md`, 2026-09-25, "The browser suites'
  signed-out states draw the sign-in prompt ...").
...
- the cloud contract over the real adapter (`tests/e2e/contract.mjs`), in
  Node: `createCloud` with a store holding a minted session, cases A-H (F,
  the preferences round trip, G, a list through every write, and H, its
  share links made, read in both projections, deleted, made again, read
  back with the stopped rows and copied, on a throwaway that case E then
  deletes with its rows), the real-only share checks on two more throwaways
  ...
  list, and copies a GM link with its GM notes and a player link with none;
  a signed-out copy refused), a create of 101
  entries on another throwaway answered as the `entries_per_list` limit of
  100 (the trigger's DETAIL through PostgREST's `details`), and anon's
  `rpc('delete_account')`, `lists` and `user_prefs` selects refused and
  `get_shared_list` of an unknown token null;
...
- the flows in Chrome over that `dist/`, each in its own browser context
  ...
  English back (`dhloot.lang.v1` `en`), F7 the member makes «E2E список» on
  `#/lists`, adds `ci1` from `#/i/ci1`'s menu, renames the list on its page,
  reads the name and the row back after a reload, and deletes it (its lists
  cleared before and after, `deleteListsOf`), F8 the member makes «E2E
  ссылки» with `ci1` and both list notes, «Поделиться» makes two active
  links, a signed-out page reads the player link (the name, the players'
  note, «Обновлено», no GM note) and the GM link (the GM note), the member's
  own page on the player link says «Это ваш список.», the member deletes the
  player link and makes a new one and deletes the GM link, signed out the
  two old links draw «Список больше не доступен» and the new one the list,
  and a throwaway's «Сохранить себе» opens a copy whose rows hold no GM
  note; rows are polled through the admin client (`prefsOf`, `listsOf`,
  `sharesOf`);
...
deletes that same port's account; E delete leaves nothing signed in. A also
requires `prefs` and `lists.list()` refused signed out, B an `ok` read for
the member. The seed's own behaviour stays in
...
- Five `tests/*.js` files, fs/node-only: `contracts` (trimmed to its pure
  ...
  committed (`docs/specs/CONTRACTS.md` section 5). `run-all.js` runs
  `node tools/build.js` once when `i/` is missing and one of these four is
  selected; a direct `node tests/<suite>.js` on a clean checkout still needs
  that build first. `npm run check` runs the build, then `derived` and
...
- `tests/db/` - layer 3, the database, run by `npm run check:db`, not by
  ...
  and no `public` function is executable by `anon` but the one listed
  exception, `get_shared_list(text)` (read with
  ...
  `delete_account()` too. `limits.test.mjs` holds the count limits: the
  two defaults, no privilege for `anon` or `authenticated` on either table
  ...
  default again once it is deleted, and `limits:set`'s database half
  ...
  move between lists included), `reorder_list()` and its refusals, and the
  rows going with their user. `list-shares.test.mjs` covers six roles: no
  token, a player link (no `gm_note` key anywhere), a GM link, the owner
  (idempotent create, stop twice, its stopped share read with
  `revoked_at` - the share panel's rule rests on it - a new link after a
  stop, no direct write), another user (no
  access, a read-only GM link, a copy with only its link's notes, counted
  against its own limits, refused signed out or under another user's list
  id) and a stopped link (null,
  no copy), with EXECUTE pinned per function. `reversibility.test.mjs`
  ...
  unchanged, and the base check resets the database once, to the first
  migration (`supabase db reset --local --version`), then walks the rest
  in order: each reversible one's up then down must leave the schema its
  predecessor left, then it is applied again; an additive one is only
  applied. The walk must end in the schema `apply-pending.test.mjs` left,
  which that test proves equal to the CLI's own reset, so the database is left fully migrated for the files after it and a
  run resets three times whatever the migration count - the first
  migration has none below it and is proven by up-down-up alone. It proves
  ...
  runs it in the `db` job, which `deploy` needs. `apply-pending.test.mjs`
  proves the applier behind CI's `migrate-test` step
  (`tools/supabase/db.mjs`, `applyPending`): from a database reset to the
  first migration it applies the rest oldest first, and every history row
  reads `<version>_<name>.sql` as the CLI's own row does, and the schema it
  leaves equals the `db dump` taken after the CLI's reset; a history version
  no local file names is listed as foreign and applies nothing; over
  `tests/db/fixtures/pending/` a file whose second statement fails stops
...
| `contracts` | contract | pure half kept; browser half ported to `tests/app/contracts.js` | golden fixtures: list encode and decode, both link variants, truncation, the equipment stat line in both languages, filter group key names against the docs, `data.json` top-level keys against `CONTRACTS.md` section 4 |
...
tests). Stage one can go red twice in a row: on the Linux cloud container on
2026-09-24 it did, with no storage code changed, and the next run and an
independent run both passed - one stage-one red after a re-run is not yet
evidence of an app defect; a stage-two red is.
...
| Share links: the panel, the shared page `#/s/<token>`, «Обновлено N назад», the owner's line, «Сохранить себе», the gone page, the `#/l/` announcement | `lib/hash.test.ts`, `lib/cloudLists.test.ts`, `lib/ago.test.ts`, `state/sharedView.test.ts`, `state/app.test.ts`, `ports/fake-cloud.test.ts` and the contract's case H, `ports/supabase.test.ts`, `ports/lazy-cloud.test.ts`, `components/sharePanel.test.ts`, `sharedListPage.test.ts`, `listPage.test.ts`, `a11y.test.ts`, `tests/app/states.js` cases 43-46, the `~ share` and `#/s/` goldens, `tests/app/contracts.js` (the two `#/s/` fixtures), `tests/contracts.js`, `tests/db/list-shares.test.mjs`, `tests/e2e/run.mjs` (case H, the real-only share checks, flow F8) |
| Account lists: groups, "edited N ago", the prompt and the return, save status, delete, limits | `lib/cloudLists.test.ts`, `lib/ago.test.ts`, `lib/pending.test.ts`, `state/cloudLists.test.ts`, `state/app.test.ts`, `ports/fake-cloud.test.ts` and the contract's case G, `ports/supabase.test.ts`, `ports/lazy-cloud.test.ts`, `ports/redirect.test.ts`, `components/listsPage.test.ts`, `listPage.test.ts`, `lists.test.ts`, `sharedListPage.test.ts`, `signInPrompt.test.ts`, `tests/app/states.js` cases 37-42, the `#/lists as gm1`, `~ load failed`, account list page and prompt goldens, `tests/db/lists.test.mjs`, `tests/e2e/run.mjs` (case G, flow F7) |
...
| Fake cloud and the test build | `ports/fake-cloud.test.ts` (the cloud contract, `?as=`, the lists repository and its failure switch), `tests/app/states.js` case 35, `tools/no-fake-in-prod.mjs` |
...
static tree could never see - homed in `DEBT.md` at the time (every entry
since paid, moved or dropped; `git show 7b0def9:docs/specs/DEBT.md`) and "Known thin
spots" below.
...
D, F), the classes no instrument could see. `DEBT.md` recorded the 82 homed
rows, not the 519 read; every one is since paid, moved or dropped.
...
components in isolation. The check prints only the coverage summary; the
per-file table prints with `npx vitest run --coverage
...
| `components/lists.test.ts` | the add-to-list row itself - `listMenuHTML`/`addToListBtn` and `listMemberFor(item)`'s live behaviour: the button, the menu, a chip's tick, the new-list form, and the toast each raises; over the fake cloud, the prompt in the new-list slot (focused, the chips still pickable), no chip while the session is unknown, the return after sign-in from a card, the bar and the record dialog with the lists holding the record first, the typed name kept through the sign-in, and account lists made, added to and removed from with an undo |
...
| `ports/fake-cloud.test.ts` | `ports/cloud.contract.ts`, the contract every `CloudPort` meets - `auth` and `prefs`, cases A-F, `lists`, case G, and `shares`, case H ("Test layers", "The contract selection") - run over the fake here and over the real adapter in layer 4; the seed's own cases (a sign-in as the default user notifying once, the identities in the seed's order, unlink then the last refused, link, a port made as `gm2`, `gm1`'s preferences row and none for `gm2`, a saved row kept to its own port, no read after deletion); the seeded lists and shares pinned (every record key in `data.json`, unique ids, quantities, prices and positions in bounds, one active share per audience); plus the fake's own refusals, its test options (`linkError` refuses a link and changes nothing, `returned` is the redirect result as given, `offline` and `limits`), the lists repository (`gm1`'s three lists with times back from boot and entries in position order, ids from `uuid(5000)`, every write bumping the edit time, an unknown row left alone, another user's list, a second entry for one record and a short reorder refused, the 51st list and the 101st entry, offline and signed out), the share links (the seed links read signed out with no `gm_note` key on the player one, the owner answered to `gm1` only, a list's shares to its owner only, another user's create refused, new shares counted from `share-token-1`, a removed list's link reading nothing, a GM copy with its notes and the list limit, offline), an unknown `?as=` user throwing, and `installFakeCloud` |
| `ports/redirect.test.ts` | the provider redirect's way back over a stub window: the callback address, the saved record, and `takeRedirect` - no flag, a code, `error_code` over `error`, an error or tokens in the hash, a stale, future, foreign, overlong (16384) or malformed record, a 3000-character address and a prompt's action returned, a bad action dropped with the way back kept, a storage that throws, and the cleaned address in each case |
| `ports/supabase.test.ts` | the real adapter over a stub supabase-js client: the PKCE client options, the session and identity mapping, the code exchanged before the session is read, every refusal mapped (`identity_already_exists`, `single_identity_not_deletable`, a throw), identities `[]` signed out and `null` when a signed-in read fails, the way back saved before the provider call, a started redirect kept pending until a back-forward-cache `pageshow` while a refused one answers at once, a store handed in, sign-out scopes, delete then a local sign-out whatever it answers, auth changes forwarded, a prompt's page and action saved and returned by `redirectResult()`; the lists: one read with its entries embedded and sorted by position then id, not `ok` on an error, a throw or no rows, `create` as two upserts that ignore a row already there (none for no entries or a refused list), each write on its row, `reorder` through `reorder_list`, the write classifier (`limit: <key>` with its number, a status 0 or 5xx or a throw as `network`, any other error `refused`) and a fresh v4 id; the preferences row: nothing read or written signed out, the user's own row read with an invalid field dropped, no row as `null`, a failed or thrown read not `ok`, the whole row upserted on `user_id` and a refusal or a throw answered false |
| `ports/lazy-cloud.test.ts` | the lazy port: loaded once on first use, every method delegated, a loaded port's `null` identities passed through, a subscription made when the port arrives or cancelled before, and a failed load answering signed out and refusing (preferences unread, a save refused, the lists unread and every list write `network`); a loaded port's preference and list answers passed through, and `newId()` answering before the load |
| `components/accountPage.test.ts` | `#/account` over the fake cloud: the chooser and its consent links in both languages, sign-in (a prompt's page and action handed to the port, and the return to that page), the redirect state, only the title while the session is unknown, the four sections as `gm1`, disconnect and its hint, connect and `redirecting`, the already-linked alert from `link()` and from the redirect, sign out and everywhere (`'global'`), delete with the typed word (trimmed, any case), cancel, every refusal's toast, identities that cannot be read (the alert, no Connect), an account with no provider identity or no email, English, and axe in each state |
...
| `state/app.test.ts` | settings read as untrusted data, which address may be pinned, a refused write, what an old section name sets, the storage notice's own dismissal flag, a packed address expanded through the compress port, and where it lands when the port cannot; the print layout in `dhloot.prefs.v1` (read at boot, live's `{ view }` shape, broken JSON, written whole); account preferences (the account wins and saves nothing back, a `home` pinned without navigating, removed at the default or ignored when refused, a first sign-in seeding five values, a failed read applying nothing, each setter saving the whole object once signed in and none signed out, a pending read losing to a local edit, sign-out clearing nothing, another user pulling again, the shown-again signal pulling or, after a refused save, saving); account lists (where a list is made in each session state, the lists read on sign-in and `storeFor`, a prompt remembered until `#/account` is left, the return after sign-in with the bar's rows, taken counts and menu or a card's menu, a redirect's action run once the lists are read, an old link saved into the account and opened, a packed link awaited, sign-out off an account list's page, the shown-again signal and the 45 s re-read on the index and an account list's page only) |
| `lib/cloudLists.test.ts`, `lib/ago.test.ts`, `lib/pending.test.ts` | an account list's rows mapped to the local shape (order by position then id, a key only when set), the rows of new entries, the limit texts, a share link's projection mapped to the shared page's shape (the link's notes only, an unknown record dropped and counted), the active share of an audience over its stopped ones; "edited N ago" in both languages at every step and "Updated N ago" at two; a pending action read as untrusted data |
| `state/sharedView.test.ts` | the open share link's list over the fake: loading then ready, gone for an unknown token, error offline and ready after `retry()`, a re-read that finds nothing new keeping the same object, the owner's edit and a deleted link re-read, a failed re-read keeping the row, the owner check per user id, an answer for a closed token dropped, `close()` |
| `components/sharePanel.test.ts` | the share panel through `App` as `gm1`: both seed links and no create, both links made on open for a list with none, a deleted link kept deleted on the next open until «Создать ссылку», «Скопировать» writing `<base>#/s/<token>` with each row's toast, a failed change keeping the row, a refused one reloading the panel, a failed read and «Повторить»; axe open, deleted and failed |
| `state/cloudLists.test.ts` | the account list store over the fake: each writer's effect equal to the local store's on the same input and the fake holding the result, writes in order with a queued edit of one field sent once, a lost network kept and retried every 15 s, on `retry()` and on `refresh()`, a limit and a refusal toasted and re-read (one toast per flush, so a refused add's reorder does not replace the limit text), a refused create taking its queued writes along, `clear()`, a read overlapping an edit read again, and a re-read that finds nothing new keeping the same array and lists while an edit replaces only its own list |
| `components/signInPrompt.test.ts` | the sign-in prompt: one line and one «Войти» that remembers the page and the action and opens `#/account`, «Отмена» in the compact form only, and axe on the plain, compact and boxed forms |
...
| `components/listsPage.test.ts` | `#/lists` - off `renderLists`/`storageWarning`/`listCardHTML` and the create/share/delete handlers (app.js 2865-2931, 4136-4270): the head and its help, the storage notice (`StorageNotice.svelte`, extracted on its second use in B5.4) in both live forms, a card per list with a known-record badge and its actions, the name filter from the eighth list, the first 24 cards and «Показать ещё (N)» with its focus move and its count kept across a return, and `noData`; over the fake cloud, the prompt panel signed out (no browser group without browser lists, the unreadable notice still drawn), nothing while the session is unknown, the two groups with «изменён N назад», an account create and delete (the confirm naming the links, no undo), loading, a failed read and its retry, an empty account, and the filter and the fold over both groups |
| `components/listPage.test.ts` | `#/lists/<id>` and `#/l/<own payload>` - off `renderOneList` and everything it draws (app.js 2933-3128, the address at 1537-1607, the handlers at 3944-4571): the address rewrite and its address-only edge cases, the title input and the actions row, the storage notice, the money picker and its help, the list note, the roll panel (folded/absent/rolled/reset), select-all, a row's position/qty/gold/note/remove with undo, copying and sharing, deleting, a row's own modal, the drag port's `onDrop`, `noData`, and the branch into the shared page (drawn for a payload that is nobody's, nothing drawn while a packed address expands, the bad-link page when the port cannot expand one); an account list over the fake cloud: its address kept, no link buttons or notice, «Сохраняем...» / «Сохранено» / «Не сохранено» and «Повторить» in the sub with only the failure announced, delete with the confirm and no undo, and the not-found states of an account address (signed out, pending, failed, not held) |
| `components/sharedListPage.test.ts` | `#/l/<payload>` for a payload that is nobody's own list - off `renderSharedList` (app.js 3130-3170): the heading and the one-text-node sub, the save button and saving the whole list as a new own list (both notes, the money mode, a nameless list), the bar alone holding add-to-list with a row ticked, and the bar taking rows into a new or an existing list (qty/gold/note, never the GM's own note), both list hitnotes and each entry's own after its row, the tails (`×qty`, `×qty · price`, bare price, coin mode), selection and the bar it raises, a row's own modal, the bad-link page, and English; over the fake cloud, «Сохранить себе» saving into the account signed in, pressed with the prompt under it signed out (a second press folds it; the sign-in saves and opens the copy; no browser list made), and nothing while the session is unknown |
...
sharing record), the thumbnail destination `img/thumb/<asset>` in both
...
computed style. A CSS-only change can therefore
...
- No harness exercises the real install prompt, the installed standalone
  ...
  whether Safari reads the manifest link). `ports.test.ts` covers `standalone()`
...
- No golden captures an English load-time toast: a toast raised on load
  fires before the harness switches to English, so the EN half of
  `_l_every_entry_gone.txt` and `_lists_a_removed.txt` reads the Russian
  text. The English strings are covered by the unit suite only.
...
asks, with **no known divergence** between the two apps (unlike the rows
`DEBT.md` once held, where a divergence *was* found). Full detail, row by row,
